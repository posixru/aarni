...
quint32 AES::encrypt(const QByteArray& in, QByteArray& out)
{
    ...
    while (32 <= left)
    {
        xtsEncryptBlock(tweak, input, output);
        input += 16;
        output += 16;
        left -= 16;
        xtsComputeTweak(tweak);
    }

    // encrypt the last two blocks
    if (16 == left)
    {
        // data unit is a multiple of 128-bit blocks
        xtsEncryptBlock(tweak, input, output);
    }
    else
    {
        ...
        memcpy(PP, input + 16, left);
        memcpy(PP + left, CC + left, 16 - left);
...
quint32 AES::decrypt(const QByteArray& in, QByteArray& out)
{
    ...
    while (32 <= left)
    {
        xtsDecryptBlock(tweak, input, output);
        input += 16;
        output += 16;
        left -= 16;
        xtsComputeTweak(tweak);
    ...
    else
    {
        ...
        left -= 16;
        memcpy(output + 16, PP, left);
        memcpy(CC, input + 16, left);
        memcpy(CC + left, PP + left, 16 - left);
